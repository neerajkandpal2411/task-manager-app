      ...
      <div className="glass-card rounded-xl p-4 animate-slideIn">
        ...
        <div className="flex gap-2">
          <button
            ...
            className="flex-1 bg-orange-600/80 hover:bg-orange-600 text-white py-2 rounded-lg transition-all duration-300 hover:scale-105 flex items-center justify-center gap-2"
          >
      ...
      <div className="flex items-start gap-3">
        <button
          ...
          className={`mt-1 w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all duration-300 
            ${task.completed 
              ...
              : 'border-orange-500/50 hover:border-orange-500 hover:scale-110'}`}
        >
          {task.completed && <Check className="w-4 h-4 text-white" />}
        </button>
        
        <div className="flex-1">
          <h3 className={`font-semibold text-lg transition-all duration-300 ${task.completed ? 'line-through text-orange-500/40' : 'text-white'}`}>
            {task.title}
          </h3>
          {task.description && (
            <p className={`text-orange-300/60 text-sm mt-1 transition-all duration-300 ${task.completed ? 'line-through' : ''}`}>
              {task.description}
            </p>
          )}
          {task.dueDate && (
            <div className={`flex items-center gap-2 mt-2 text-xs ${isOverdue ? 'text-red-400' : 'text-orange-500/50'}`}>
        ...
        <div className="flex gap-2">
          <button
            ...
            className="text-orange-400 hover:text-orange-300 transition-all duration-300 hover:scale-110 p-1"
            title="Edit task"
          >
